...
- **LLM**: Sends prompts to `http://localhost:8080/v1/chat/completions` (e.g. local LLM server). System prompt is **`SAR_SYSTEM_PROMPT`** in `src/llm.rs` (example-first JSON `tasks` router, max **5** steps). The LLM returns `{"tasks":[...]}`. Responses include **`pending_approval`**: when **`true`**, the client should show approval before **ApplyTool** / **ApplyToolSequence**; today Infer **auto-applies** and returns **`false`**. Multi-step plans include **`tools`**: an ordered array of steps. Measured GGUF comparison (we serve FT Qwen3.5-0.8B e3): [docs/llm-bench-v1-model-comparison.md](docs/llm-bench-v1-model-comparison.md).
