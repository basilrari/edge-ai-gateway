//! Relay browser WebSocket ↔ drone-http telemetry stream.

use axum::extract::ws::{Message, WebSocket};
use futures_util::{SinkExt, StreamExt};
use tracing::warn;

pub async fn relay_telemetry_ws(mut client: WebSocket, drone_ws_url: String) {
    let drone_connect = match tokio_tungstenite::connect_async(&drone_ws_url).await {
        Ok(pair) => pair,
        Err(e) => {
            warn!(error = %e, url = %drone_ws_url, "drone telemetry ws connect failed");
            let _ = client
                .send(Message::Text(
                    format!(r#"{{"error":"drone_ws_connect_failed","detail":"{e}"}}"#).into(),
                ))
                .await;
    ...
    let client_to_drone = async {
        while let Some(msg) = client_stream.next().await {
            match msg {
                ...
                Ok(Message::Ping(p)) => {
                    if drone_sink.send(tokio_tungstenite::tungstenite::Message::Ping(p))
                        .await
                        .is_err()
                    {
                        break;
                ...
                Ok(Message::Pong(p)) => {
                    if drone_sink.send(tokio_tungstenite::tungstenite::Message::Pong(p))
                        .await
                        .is_err()
                    {
                        break;
